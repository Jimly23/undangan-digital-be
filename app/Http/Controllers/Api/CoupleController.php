...
class CoupleController extends Controller
{
    ...
    // 1. Validasi field selain file musik (jika ada tambahan field required silakan masukkan ke sini)
    $request->validate([
        'template' => 'required',
        'slug' => 'required',
        // 'musik' dihilangkan dari sini agar tidak terkena error mimetypes bawaan Laravel
    ]);

    // 2. Validasi manual khusus untuk file musik (Bypass Masalah MIME Type)
    if ($request->hasFile('musik')) {
        $fileMusik = $request->file('musik');
        $ekstensi = strtolower($fileMusik->getClientOriginalExtension());

        // Cek kecocokan ekstensi yang diizinkan
        if (!in_array($ekstensi, ['mp3', 'wav', 'ogg'])) {
            return response()->json([
                'message' => 'The musik field must be a file of type: mp3, wav, ogg.',
                'errors' => [
                    'musik' => ['The musik field must be a file of type: mp3, wav, ogg.']
                ]
            ], 422);
        }

        // Cek ukuran file (Maksimal 20MB = 20480 KB)
        if ($fileMusik->getSize() > 20480 * 1024) {
            return response()->json([
                'message' => 'The musik field must not be greater than 20 megabytes.',
                'errors' => [
                    'musik' => ['The musik field must not be greater than 20 megabytes.']
                ]
            ], 422);
        }
    } else {
        // Jika field musik bersifat wajib diisi (required)
        return response()->json([
            'message' => 'The musik field is required.',
            'errors' => [
                'musik' => ['The musik field is required.']
            ]
        ], 422);
    }
    ...
    // Proses simpan file musik setelah lolos validasi manual di atas
    $musik = null;
    if ($request->hasFile('musik')) {
    ...
    // Ubah string hobi/tamu menjadi array
